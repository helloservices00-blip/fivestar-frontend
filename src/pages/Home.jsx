  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <button onClick={() => { setActiveModule(null); setActiveCategory(null); setActiveSubcategory(null); }} className="px-3 py-1 bg-blue-500 text-white rounded">All</button>
        {modules.map((m) => (
          <button key={m._id} onClick={() => { setActiveModule(m._id); setActiveCategory(null); setActiveSubcategory(null); }} className={`px-3 py-1 rounded ${activeModule === m._id ? 'bg-blue-400 text-white' : 'bg-gray-200 hover:bg-gray-300'}`}>{m.name}</button>
        ))}
      </div>
        ...
        <>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {categories
              ...
              .map(c => (
                <button key={c._id} onClick={() => { setActiveCategory(c._id); setActiveSubcategory(null); }} className={`px-3 py-1 rounded ${activeCategory === c._id ? 'bg-blue-400 text-white' : 'bg-gray-200 hover:bg-gray-300'}`}>{c.name}</button>
              ))}
          </div>
        </>
      )}
        ...
        <>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {subcategories
              ...
              .map(s => (
                <button key={s._id} onClick={() => setActiveSubcategory(s._id)} className={`px-3 py-1 rounded ${activeSubcategory === s._id ? 'bg-blue-400 text-white' : 'bg-gray-200 hover:bg-gray-300'}`}>{s.name}</button>
              ))}
          </div>
        </>
      )}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map(p => (
            <Link to={`/product/${p._id}`} key={p._id} className="border rounded p-3 hover:shadow transition">
